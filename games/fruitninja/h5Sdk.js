  ...
  function _send(data, extraData) {
    try {
      const stringData = JSON.stringify(
        Object.assign(data, (typeof extraData === "object" && extraData) || {}),
      );
      ...
      if (window.postMessage) {
        ...
        if (document.referrer) {
          ...
          if (
            origin &&
            (/^http[s]?:\/\/(.+\.)?storms\.com(:\d{2,6})?[/]?$/gi.test(
              origin,
            ) ||
              ...
              /^http[s]?:\/\/(.+\.)?kyo\.games(:\d{2,6})?[/]?$/gi.test(
                origin,
              ) ||
              /^http[s]?:\/\/(.+\.)?voluble-croissant-15cf07\.netlify\.app(:\d{2,6})?[/]?$/gi.test(
                origin,
              ))
          ) {
