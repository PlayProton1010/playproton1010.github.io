...
var LaunchDungeons = function () {
  ...
  Game.Entity = function (type, subtype, dungeon, pic, stats) {
    //objects you could find on the map : doors, mobs, interactables, items, player, exits...
    this.type = type;
    ...
    this.Draw = function () {
      //return the string to draw this
      var name = "?";
    ...
    this.Wander = function () {
      //AI to move around aimlessly
      this.targets = [];
      this.targets.push([-1, 0], [1, 0], [0, -1], [0, 1]);
      this.Move();
    };
    this.GoTo = function (x, y) {
      //AI to move to a specific point
      this.targets = [];
    ...
    this.Flee = function (x, y) {
      //AI to run away from a specific point
      this.targets = [];
      if (this.x > x) this.targets.push([1, 0]);
      if (this.x < x) this.targets.push([-1, 0]);
      if (this.y > y) this.targets.push([0, 1]);
    ...
    this.Turn = function () {
      //do this every turn (walk around, heal up...)
      if (this.type == "monster") {
        ...
        for (var i = 0; i < howManyTurns; i++) {
          if (1 == 1) {
            //this.AI!='static')
            if (this.AI == "flee")
              this.Flee(this.dungeon.heroEntity.x, this.dungeon.heroEntity.y); //flee from the player
            else {
  ...
  Game.Dungeon = function (type, id) {
    ...
    this.GetEntities = function (x, y) {
      //returns the first entity found on tile x,y
      var entities = [];
    ...
    this.CheckObstacle = function (x, y) {
      //returns 0 for no obstacle; -1 for a wall; an entity if there's at least one entity on this tile
  ...
  Game.DungeonLocationChain = function (map, x, y) {
    //return an array of the rooms between the root room and this tile's room, inclusive
  ...
  Game.DungeonLinkLocationChains = function (start, end) {
    //return the room in which the first location chain should go to to get closer to the second location chain
    /*
		4 cases
