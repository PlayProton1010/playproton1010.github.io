pc.script.create("ui_ingame", function (context) {
  ...
  Ui_ingame.prototype = {
    initialize: function () {
      ...
      this.fuelProgress =
        this.entity.findByName("ProgressFuel").script.progressbar;
      this.bonusProgress =
        this.entity.findByName("ProgressBonus").script.progressbar;

      this.gameoverSprite = this.entity
        .getParent()
        .findByName("SpriteGameover");
      ...
      for (var i = 0, len = this.game.maxStreak; i < len; i++) {
        var sprite = this.entity
          .getParent()
          .findByName("SpriteStreak" + (i + 1).toString());
        if (sprite) {
    ...
    onGameOver: function (score, isHighScore) {
      ...
      setTimeout(
        function () {
          this.gameoverSprite.enabled = false;
          var gameOverScreen =
            this.getUi().showScreen("gameover").script.ui_gameover;
