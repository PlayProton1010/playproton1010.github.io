...
pc.script.create("patterns", function (context) {
  ...
  Patterns.prototype = {
    postInitialize: function () {
      ...
      this.spawnables = {};
      spawnables.getChildren().forEach(
        function (child) {
      ...
      patternsEntity.getChildren().forEach(
        function (pattern) {
      ...
      for (var i = 0, len = this.patterns.length; i < len; i++) {
        if (
          this.patterns[i].entity.getName() === this.triggerBonusModePattern
        ) {
    ...
    getAngleToTravelIfPatternsBlocked: function (angle) {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (
          (angle >= block.fromAngle && angle <= block.toAngle) ||
    ...
    findMatchingPattern: function (previousPattern) {
      if (this.useTriggerBonusModePattern) {
        ...
        var candidates = this.patterns.filter(
          function (pattern) {
            ...
            if (
              previousPattern &&
              previousPattern !== this.patternThatTriggersBonusMode
            ) {
    ...
    update: function (dt) {
      ...
      this.getNormalizedPlayerPosition(
        this.normalizedPlayerPosition,
        playerPos,
      );
    ...
    spawnNewPatternIfNeeded: function (dt, playerPosition) {
      this.angleTravelled =
        Math.acos(
          this.normalizedPlayerPosition.dot(this.lastNormalizedPlayerPosition),
        ) * pc.math.RAD_TO_DEG;
    ...
    calculateNewPatternPosition: function (playerPosition, result) {
      result
        .cross(pc.Vec3.UP, this.normalizedPlayerPosition)
        .scale(50)
        .add(playerPosition);
      if (!this.inBonusMode) {
    ...
    spawnPattern: function (topLeft) {
      ...
      for (index = 0; index < maxChildren; index++) {
        ...
        var childAngle = this.getAngleFromChordLength(
          radius,
          localPosition.x * pattern.horizontalScale,
        );
      ...
      this.angleToBeTravelledBeforeSpawn = this.getAngleFromChordLength(
        radius,
        pattern.width * pattern.horizontalScale + pattern.spaceAfter,
      );
    },
