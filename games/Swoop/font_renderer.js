  ...
  Font_renderer.prototype = {
    initialize: function () {
      ...
      if (!vertexFormat) {
        vertexFormat = new pc.gfx.VertexFormat(gd, [
          ...
          {
            semantic: pc.gfx.SEMANTIC_TEXCOORD0,
            components: 2,
            type: pc.gfx.ELEMENTTYPE_FLOAT32,
          },
        ]);
      ...
      context.assets.load(assets).then(
        function (resources) {
          this.atlas = resources[0];
          this.font = resources[1];

          // Create a vertex buffer
          this.vertexBuffer = new pc.gfx.VertexBuffer(
            gd,
            vertexFormat,
            6 * this.maxTextLength,
            pc.gfx.BUFFER_DYNAMIC,
          );
          this.updateText(this.text);

          var command = new pc.scene.Command(
            pc.scene.LAYER_HUD,
            pc.scene.BLEND_NORMAL,
            function () {
              if (this.entity.enabled) {
                ...
                gd.setBlending(true);
                gd.setBlendFunction(
                  pc.gfx.BLENDMODE_SRC_ALPHA,
                  pc.gfx.BLENDMODE_ONE_MINUS_SRC_ALPHA,
                );
                ...
                gd.scope
                  .resolve("uScale")
                  .setValue(this.calculateScaling().data);
                gd.scope
                  .resolve("uOffset")
                  .setValue(this.calculateOffset().data);
    ...
    updateText: function () {
      ...
      for (i = 0; i < textLength; i++) {
        ...
        // Add vertices
        iterator.element[pc.gfx.SEMANTIC_POSITION].set(
          tempCursorX,
          tempCursorY - height,
        );
        ...
        if (i == textLength - 1) {
          ...
          kerning =
            this.font.kernings[charId] && this.font.kernings[charId][nextId]
    ...
    calculateOffset: function () {
      ...
      this.offset
        .set(this.x * this.scaling.x, this.y * this.scaling.y)
        .add(this.userOffset)
