...
pc.script.create("player", function (context) {
  ...
  Player.prototype = {
    ...
    initialize: function () {
      ...
      this.obb = new pc.shape.Box(
        this.entity.getWorldTransform(),
        new pc.Vec3(3, 0.5, 0.5),
      );
    ...
    updateSpeed: function (dt) {
      ...
      if (this.tilt < 0 && this.tilt > -180) {
        ...
        accelerationFactor = pc.math.lerp(
          this.minDecelerationFactor,
          this.maxDecelerationFactor,
          adjustedTilt / 90,
        );
      }

      var minSpeed = this.game.inBonusMode
        ? this.bonusSpeedModifier * this.minSpeed
        : this.minSpeed;
      var maxSpeed = this.game.inBonusMode
        ? this.bonusSpeedModifier * this.maxSpeed
        : this.maxSpeed;
      this.speed = pc.math.clamp(
        this.speed + accelerationFactor * this.fallAcceleration * dt,
        minSpeed,
        maxSpeed,
      );
    },
