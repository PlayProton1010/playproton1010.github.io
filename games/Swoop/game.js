    ...
    update: function (dt) {
      ...
      if (!this.isOver) {
        ...
        if (this.inBonusMode) {
          this.bonusTimer -= dt;
          this.setBonusMeter(
            pc.math.lerp(0, 100, this.bonusTimer / this.bonusModeDuration),
          );
          ...
          this.setFuel(
            this.fuel -
              (this.fuelDecreaseRate +
                (this.difficulty - 1) * this.fuelDifficultyModifier) *
                dt,
          );
        }
    ...
    onPickedCollectible: function (collectible) {
      ...
      if (collectible.effectName) {
        this.particles.spawn(
          collectible.effectName,
          collectible.entity.getPosition(),
        );
      }
