    ...
    Bootstrap.prototype = {
      ...
      _createApplication: function (content) {
        ...
        try {
          ...
          this.application.setCanvasResolution(
            this.appProperties["resolution_mode"],
            this.appProperties["width"],
            this.appProperties["height"],
          );
          this.application.setCanvasFillMode(
            this.appProperties["fill_mode"],
            this.appProperties["width"],
            this.appProperties["height"],
          );
      ...
      _startFromDesigner: function (packId) {
        var self = this;
        self.server = new pc.common.Corazon(
          pc.config["api_url"],
          pc.config["corazon"],
        );

        self.server.authorize(self.username, function () {
          self.server.users.getOne(
            self.username,
            function (user) {
              user.depots.getOne(
                self.depotName,
      ...
      _onWindowResize: function () {
        var size = this.application.resizeCanvas(
          this.canvas.width,
          this.canvas.height,
        );
...
var progress = new pc.ProgressBar(
  document.getElementById("application-progress"),
  2,
);
