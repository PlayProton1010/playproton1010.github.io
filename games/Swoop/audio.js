...
pc.script.create("audio", function (context) {
  ...
  Audio.prototype = {
    ...
    initialize: function () {
      ...
      this.sounds = [this.engineSource].concat(this.genericSoundSources);
      ...
      this.game = context.root.findByName("Game").script.game;
      this.musicVolume = this.game.storage.loadNumber(
        "music",
    ...
    onCollectible: function (collectible) {
      ...
      if (collectible.triggerBonusMode) {
        ...
        sound = pc.string.format(
          "{0}_{1}",
          this.megaGemSound,
          this.megaGemSoundIndex + 1,
        );
        this.megaGemSoundIndex =
          (this.megaGemSoundIndex + 1) % this.numMegaGemSounds;
        this.megaGemSoundTimer = this.resetNotesTime;
      } else {
        sound = pc.string.format(
          "{0}_{1}",
          this.simpleGemSound,
          this.simpleGemSoundIndex + 1,
        );
        this.simpleGemSoundIndex =
          (this.simpleGemSoundIndex + 1) % this.numSimpleGemSounds;
    ...
    getFreeAudioSource: function () {
      ...
      this.currentAudioSource =
        (this.currentAudioSource + 1) % this.genericSoundSources.length;
      return result;
    },
    ...
    reset: function () {
      ...
      this.lastPlayerPosition.copy(
        this.game.player.entity.getPosition().clone(),
      );
    },

    onCollision: function (isSolidObstacle) {
      this.playOneOffSound(
        isSolidObstacle ? this.groundHitSound : this.cloudHitSound,
      );
    },
