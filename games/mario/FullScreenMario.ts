...
namespace FullScreenMario {
   ...
   */
  export class FullScreenMario
    extends GameStartr.GameStartr
    implements IFullScreenMario
  {
    ...
    public static gravity: number =
      Math.round(12 * FullScreenMario.unitsize) / 100;
    ...
    resetObjectMaker(
      FSM: FullScreenMario,
      settings: GameStartr.IGameStartrSettings,
    ): void {
    ...
    resetItemsHolder(
      ...
      if (settings.width < 560) {
        (<HTMLElement>(
          (<HTMLTableRowElement>FSM.ItemsHolder.getContainer().children[0])
            .cells[4]
        )).style.display = "none";
      }
    }
    ...
    resetMathDecider(
      FSM: FullScreenMario,
      customs: GameStartr.IMathDecidrCustoms,
    ): void {
    ...
    gameStart(): void {
      var FSM: FullScreenMario =
        FullScreenMario.prototype.ensureCorrectCaller(this);
    ...
    gameOver(): void {
      ...
      FSM.TimeHandler.addEvent(function (): void {
        FSM.gameStart();
    ...
    generateThingKey(thing: IThing): string {
      return (
        (<IArea>thing.GameStarter.AreaSpawner.getArea()).setting +
        " " +
        thing.groupType +
        " " +
        thing.title +
    ...
    addPreThing(prething: IPreThing): void {
      ...
      thing.FSM.addThing(
        ...
        ((<IMapScreenr>thing.FSM.MapScreener).floor - prething.top) *
          thing.FSM.unitsize,
      );
          ...
          switch (position) {
            case "beginning":
              thing.FSM.arrayToBeginning(
                thing,
                <any[]>thing.FSM.GroupHolder.getGroup(thing.groupType),
              );
              break;
            case "end":
     ...
     */
    addPlayer(
      left: number = this.unitsize * 16,
      bottom: number = this.unitsize * 16,
    ): IPlayer {
      var FSM: FullScreenMario =
          FullScreenMario.prototype.ensureCorrectCaller(this),
        player: IPlayer;
    ...
    maintainOverlaps(character: ICharacterOverlapping): void {
      ...
      if (character.checkOverlaps) {
        if (
          !character.FSM.setOverlapBoundaries(<ICharacterOverlapping>character)
        ) {
          return;
        }
      }
     ...
     */
    generateIsCharacterTouchingCharacter(): (
      thing: ICharacter,
      other: ICharacter,
    ) => boolean {
      /**
       ...
       */
      return function isCharacterTouchingCharacter(
        thing: ICharacter,
        other: ICharacter,
      ): boolean {
     ...
     */
    generateIsCharacterTouchingSolid(): (
      thing: ICharacter,
      other: ISolid,
    ) => boolean {
      /**
       ...
       * @returns Whether thing is touching other.
       */
      return function isCharacterTouchingSolid(
        thing: ICharacter,
        other: ISolid,
      ): boolean {
    ...
    playerAddRestingStone(thing: IPlayer): void {
      var stone: IRestingStone = <IRestingStone>(
        thing.FSM.addThing(
          "RestingStone",
          thing.left,
    ...
    spawnScalePlatform(thing: IPlatform): void {
      var collection: any = thing.collection || {},
        ownKey: string =
          thing.collectionKey === "platformLeft" ? "Left" : "Right",
    ...
    generateHitCharacterSolid(): (thing: ICharacter, other: ISolid) => void {
       ...
       */
      return function hitCharacterSolid(
        thing: ICharacter,
        other: ISolid,
      ): void {
    ...
    generateHitCharacterCharacter(): (
       ...
       * @returns Whether thing is hitting other.
       */
      return function hitCharacterCharacter(
        thing: ICharacter,
        other: ICharacter,
      ): void {
    ...
    animateLakituThrowingSpiny(thing: ILakitu): boolean {
      ...
      thing.FSM.TimeHandler.addEvent(function (): void {
        ...
        var spawn: ISpinyEgg = <ISpinyEgg>(
          thing.FSM.addThing("SpinyEgg", thing.left, thing.top)
        );
    ...
    animateSpinyEggHatching(thing: ISpinyEgg): void {
      ...
      var spawn: ISpiny = <ISpiny>(
        thing.FSM.addThing("Spiny", thing.left, thing.top - thing.yvel)
      );
     ...
     *                     (by default, false).
     */
    animateCharacterResumeVelocity(
      thing: ICharacter,
      noVelocity?: boolean,
    ): void {
      if (!noVelocity) {
    ...
    killReplace(
      ...
      title: string,
      attributes: any = {},
      attributesCopied?: string[],
    ): IThing {
      var spawn: IThing, i: number;
    ...
    killKoopa(thing: IKoopa, big?: boolean): ICharacter {
      ...
      if (thing.jumping || thing.floating) {
        spawn = <ICharacter>(
          thing.FSM.killReplace(thing, "Koopa", undefined, [
            "smart",
            "direction",
            "moveleft",
          ])
    ...
    killLakitu(thing: ILakitu): void {
      var characters: ICharacter[] = <ICharacter[]>(
    ...
    killToShell(thing: ICharacter, big?: number): IShell {
      var spawn: IShell, nocollidecharold: boolean, nocollideplayerold: boolean;
    ...
    killPlayer(thing: IPlayer, big?: number): void {
      ...
      if (big === 2) {
        ...
        if (!big && thing.power > 1) {
          ...
          FSM.arrayToEnd(
            thing,
            <any[]>FSM.GroupHolder.getGroup(thing.groupType),
          );
    ...
    setMap(name?: string | IFullScreenMario, location?: string | number): void {
      ...
      time =
        (<IArea>FSM.AreaSpawner.getArea()).time ||
        (<IMap>FSM.AreaSpawner.getMap()).time;
    ...
    setLocation(name: string | number = 0): void {
      ...
      FSM.PixelDrawer.setBackground(
        (<IArea>FSM.AreaSpawner.getArea()).background,
      );
    ...
    mapEntrancePipeVertical(FSM: FullScreenMario, location?: ILocation): void {
      ...
      FSM.AudioPlayer.addEventListener("Pipe", "ended", function (): void {
        FSM.AudioPlayer.playTheme();
      });
     ...
     */
    mapAddStretched(
      prethingRaw: string | MapsCreatr.IPreThingSettings,
    ): IThing {
      var FSM: FullScreenMario =
          FullScreenMario.prototype.ensureCorrectCaller(this),
        boundaries: any = FSM.AreaSpawner.getArea().boundaries,
        prething: MapsCreatr.IPreThingSettings =
          prethingRaw instanceof String
            ? { thing: prething }
            : <MapsCreatr.IPreThingSettings>prethingRaw,
        y: number =
          ((<IMapScreenr>FSM.MapScreener).floor - prething.y) * FSM.unitsize,
     ...
     */
    cutsceneFlagpoleStartSlidingDown(
      FSM: FullScreenMario,
      settings: any,
    ): void {
      var thing: IPlayer = settings.player,
        ...
        scoreAmount: number = FSM.scorePlayerFlag(thing, height / FSM.unitsize),
    ...
    cutsceneFlagpoleFireworks(FSM: FullScreenMario, settings: any): void {
      var numFireworks: number = FSM.MathDecider.compute(
          "numberOfFireworks",
          settings.time,
        ),
      ...
      FSM.arrayToBeginning(
        flag,
        <any[]>FSM.GroupHolder.getGroup(flag.groupType),
      );
     ...
     * @param settings   Storage for the cutscene from ScenePlayr.
     */
    cutsceneBowserVictoryCollideCastleAxe(
      FSM: FullScreenMario,
      settings: any,
    ): void {
      ...
      player.FSM.TimeHandler.addEvent(function (): void {
        player.keys.run = 1;
     ...
     */
    cutsceneBowserVictoryCastleBridgeOpen(
      FSM: FullScreenMario,
      settings: any,
    ): void {
     ...
     */
    cutsceneBowserVictoryBowserFalls(
      FSM: FullScreenMario,
      settings: any,
    ): void {
    ...
    macroPlatformGenerator(
      ...
      var output: any[] = [],
        ...
        x: number = reference.x || 0,
        yvel: number = direction * FSM.unitsize * 0.42,
        i: number;
    ...
    macroWarpWorld(
      ...
      var output: any[] = [],
        x: number = reference.x || 0,
        y: number = reference.y || 0,
        textHeight: number = reference.hasOwnProperty("textHeight")
