  ...
  FullScreenMario.settings.ui = {
    ...
    schemas: [
        ...
        options: [
          ...
          },
          {
            ...
            update: (
              ...
              FSM.PixelDrawer.setFramerateSkip(
                (1 / Number(value.replace("fps", ""))) * 60,
              );
            },
            storeLocally: true,
          },
          ...
          },
          {
            ...
            source: (FSM: FullScreenMario.IFullScreenMario): boolean => false,
            enable: (FSM: FullScreenMario.IFullScreenMario): void => {
              window.ondevicemotion = <any>(
                FSM.InputWriter.makePipe("ondevicemotion", "type")
              );
            },
            disable: (FSM: FullScreenMario.IFullScreenMario): void => {
      ...
      },
      {
        ...
        options: ((
          controls: string[],
        ): UserWrappr.UISchemas.IOptionsButtonSchema[] => {
          return controls.map(
            (title: string): UserWrappr.UISchemas.IOptionsButtonSchema => {
              return {
                title: title[0].toUpperCase() + title.substr(1),
                type: "Keys",
                storeLocally: true,
                source: (FSM: FullScreenMario.IFullScreenMario): string[] => {
                  return FSM.InputWriter.getAliasAsKeyStrings(title).map(
      ...
      },
      {
        ...
        maps: {
          ...
          callback: (
            ...
            FSM.LevelEditor.setCurrentJSON(
              JSON.stringify(FSM.MapsCreator.getMapRaw(button.textContent)),
            );
          ...
          ((): UserWrappr.UISchemas.IOptionsMapGridExtra => {
            ...
            return {
              ...
              callback: (
                ...
                schema: UserWrappr.UISchemas.ISchema,
                button: HTMLElement,
                event: UserWrappr.IEvent,
              ): void => {
                var parent: HTMLElement = event.target.parentElement,
                  randomizer: HTMLInputElement = <HTMLInputElement>(
                    parent.querySelector(".randomInput")
                  );
