...
namespace AudioPlayr {
  ...
  export class AudioPlayr implements IAudioPlayr {
    ...
    constructor(settings: IAudioPlayrSettings) {
      ...
      this.getVolumeLocal =
        typeof settings.getVolumeLocal === "undefined"
          ? 1
          : settings.getVolumeLocal;
        ...
        for (i in this.sounds) {
          if (this.sounds.hasOwnProperty(i)) {
            this.sounds[i].volume =
              Number(this.sounds[i].getAttribute("volumeReal")) * volume;
          }
        }
      }
     ...
     */
    play(name: string): HTMLAudioElement {
      var sound: HTMLAudioElement, used: number;
    ...
    playTheme(name?: string, loop?: boolean): HTMLAudioElement {
      ...
      if (
        typeof this.theme !== "undefined" &&
        this.theme.hasAttribute("name")
      ) {
        delete this.sounds[this.theme.getAttribute("name")];
      }
    ...
    playThemePrefixed(
      ...
      this.addEventListener(
        prefix,
        "ended",
        this.playTheme.bind(this, prefix + " " + name, loop),
      );
     ...
     */
    private generateLibraryFromSettings(
      librarySettings: ILibrarySettings,
    ): void {
      ...
      for (directoryName in librarySettings) {
        ...
        for (j = 0; j < directorySoundNames.length; j += 1) {
          ...
          this.library[name] = directory[name] = this.createAudio(
            name,
            directoryName,
          );
        }
