...
(function (LevelEditr_1) {
  ...
  var LevelEditr = (function () {
    ...
    LevelEditr.prototype.setCurrentThing = function (title, x, y) {
      ...
      var args = this.generateCurrentArgs(),
        description = this.things[title],
        reference = this.GameStarter.proliferate(
          {
            outerok: 2,
          },
          this.getNormalizedThingArguments(args),
        ),
        thing = this.GameStarter.ObjectMaker.make(this.currentTitle, reference);
      ...
      this.currentPreThings = [
        {
          ...
          right:
            description.offsetLeft + thing.width * this.GameStarter.unitsize,
          bottom:
            (-description.offsetTop || 0) +
            thing.height * this.GameStarter.unitsize,
      ...
      this.GameStarter.TimeHandler.addEventInterval(
        function () {
          ...
          for (var i = 0; i < scope.currentPreThings.length; i += 1) {
            scope.GameStarter.shiftHoriz(
              scope.currentPreThings[i].thing,
              direction,
            );
          }
    ...
    LevelEditr.prototype.onClickEditingThing = function (event) {
      ...
      var coordinates = this.getNormalizedMouseEventCoordinates(event, true),
        x = coordinates[0],
        y = coordinates[1];
    ...
    LevelEditr.prototype.onClickEditingMacro = function (event) {
      ...
      var coordinates = this.getNormalizedMouseEventCoordinates(event),
        x = coordinates[0],
        y = coordinates[1],
        prething,
        i;
     ...
     *
     */
    LevelEditr.prototype.onClickEditingGenericAdd = function (
      x,
      y,
      title,
      args,
    ) {
      var thing = this.GameStarter.ObjectMaker.make(
          title,
        ...
        left = x - this.GameStarter.container.offsetLeft,
        top = y - this.GameStarter.container.offsetTop;
     ...
     *
     */
    LevelEditr.prototype.onMacroIconClick = function (
      title,
      description,
      options,
      event,
    ...
    LevelEditr.prototype.generateCurrentArgs = function () {
      var args = {},
        container = this.display.sections.ClickToPlace.VisualOptions,
      ...
      for (i = 0; i < children.length; i += 1) {
        ...
        switch (
          (valuer.getAttribute("data:type") || valuer.type).toLowerCase()
        ) {
          case "boolean":
          ...
          case "number":
            value =
              (Number(valuer.value) || 0) *
              (Number(valuer.getAttribute("data:mod")) || 1);
            break;
          default:
    ...
    LevelEditr.prototype.setMapName = function () {
      var name = this.getMapName(),
        map = this.getMapObject();
    ...
    LevelEditr.prototype.addAreaToMap = function () {
      var name =
          this.display.sections.MapSettings.Area.options.length.toString(),
        map = this.getMapObject();
      if (!map) {
        return;
      }
     ...
     *
     */
    LevelEditr.prototype.resetAllVisualOptionSelects = function (
      className,
      options,
    ) {
      var map = this.getMapObject(),
        elements = this.display.container.getElementsByClassName(className),
        attributes = {
    ...
    LevelEditr.prototype.getMapObjectAndTry = function (event) {
      var mapName = this.getMapName() + "::Temporary",
        mapRaw = this.getMapObject();
      if (!mapRaw) {
        return;
      }
      try {
    ...
    LevelEditr.prototype.addMapCreationThing = function (x, y) {
      var mapObject = this.getMapObject(),
        thingRaw = this.GameStarter.proliferate(
          {
            thing: this.currentTitle,
            x:
              this.getNormalizedX(x) +
    ...
    LevelEditr.prototype.addMapCreationMacro = function (x, y) {
      var mapObject = this.getMapObject(),
        macroRaw = this.GameStarter.proliferate(
          {
    ...
    LevelEditr.prototype.resetDisplayOptionsListSubOptionsThings = function () {
      var scope = this,
        ...
        argumentGetter = this.getPrethingSizeArguments.bind(this),
      ...
      this.display.sections.ClickToPlace.Things =
        this.GameStarter.createElement("div", {
          ...
          children: (function () {
            var selectedIndex = 0,
              containers = Object.keys(scope.prethings).map(function (key) {
                var prethings = scope.prethings[key],
                  children = Object.keys(prethings).map(function (title) {
                    var prething = prethings[title],
                      thing = scope.GameStarter.ObjectMaker.make(
                        title,
                        argumentGetter(prething),
                      ),
                      container = scope.GameStarter.createElement("div", {
                      ...
                      sizeMax = 70,
                      widthThing = thing.width * scope.GameStarter.unitsize,
                      heightThing = thing.height * scope.GameStarter.unitsize,
              ...
              switcher = scope.createSelect(Object.keys(scope.prethings), {
                ...
                onchange: function () {
                  ...
                  containers[switcher.selectedIndex + 1].style.display =
                    "block";
    ...
    LevelEditr.prototype.resetDisplayOptionsListSubOptionsMacros = function () {
      ...
      scope.display.sections.ClickToPlace.Macros =
            ...
            return scope.GameStarter.createElement("div", {
              ...
              children: [
                scope.GameStarter.createElement("div", {
                  ...
                  textContent: key,
                  onclick: scope.onMacroIconClick.bind(
                    scope,
                    key,
                    macro.description,
                    macro.options,
     ...
     *
     */
    LevelEditr.prototype.setVisualOptions = function (
      name,
      description,
      options,
    ) {
      var visual = this.display.sections.ClickToPlace.VisualOptions,
        ...
        clicker = this.createVisualOption.bind(this),
        scope = this;
      visual.textContent = "";
      visual.appendChild(
    ...
    LevelEditr.prototype.createVisualOptionBoolean = function () {
      var select = this.createSelect(["false", "true"], {
      ...
      return this.GameStarter.createElement("div", {
        ...
        children: (function () {
          var modReal = option.mod || 1,
            input = scope.GameStarter.createElement("input", {
              type: "Number",
              "data:type": "Number",
          ...
          if (option.Infinite) {
            var valueOld = undefined,
              infinite = scope.createSelect(["Number", "Infinite"], {
    ...
    LevelEditr.prototype.setDisplayMap = function (doDisableThings) {
      var value = this.display.stringer.textarea.value,
    ...
    LevelEditr.prototype.parseSmart = function (text) {
      var map = JSON.parse(text, this.jsonReplacerSmart),
        areas = map.areas,
        i;
      for (i in areas) {
    ...
    LevelEditr.prototype.disableAllThings = function () {
      var scope = this,
        groups = this.GameStarter.GroupHolder.getGroups(),
        i;
      for (i in groups) {
    ...
    LevelEditr.prototype.addThingAndDisableEvents = function (thing, x, y) {
      var left = this.roundTo(x, this.GameStarter.scale),
        top = this.roundTo(y, this.GameStarter.scale);
      this.GameStarter.addThing(thing, left, top);
      this.disableThing(thing);
      this.GameStarter.TimeHandler.cancelAllEvents();
      if (
        (thing.hasOwnProperty("hidden") && thing.hidden) ||
        thing.opacity === 0
      ) {
        thing.hidden = false;
        thing.opacity = 0.35;
     ...
     *
     */
    LevelEditr.prototype.getNormalizedMouseEventCoordinates = function (
      event,
      referenceThing,
    ) {
      var x = this.roundTo(event.x || event.clientX || 0, this.blocksize),
        y = this.roundTo(event.y || event.clientY || 0, this.blocksize),
        prething;
      if (referenceThing) {
    ...
    LevelEditr.prototype.getPrethingSizeArguments = function (descriptor) {
      var output = {},
        width = this.getPrethingSizeArgument(descriptor.width),
        height = this.getPrethingSizeArgument(descriptor.height);
      if (width) {
        output.width = width;
      }
      if (height) {
    ...
    LevelEditr.prototype.getPrethingSizeArgument = function (descriptor) {
      ...
      var value = descriptor.value || 1,
        mod = descriptor.mod || 1;
      if (!isFinite(value)) {
        return mod || 8;
      }
      return value * mod;
    ...
    LevelEditr.prototype.createSelect = function (options, attributes) {
      var select = this.GameStarter.createElement("select", attributes),
        i;
     ...
     *
     */
    LevelEditr.prototype.applyElementAttributes = function (
      element,
      attributes,
    ) {
      var i;
