...
namespace TouchPassr {
  ...
  export class Control<T extends IControlSchema> {
    ...
    proliferateElement(
      ...
      var setting: any, i: string, j: number;
     ...
     *          "3.5em" or "10px".
     */
    protected createHalfSizeMeasurement(
      element: HTMLElement,
      styleTag: string,
      attributeBackup: string,
    ): string {
      var amountRaw: string, amount: number, units: string;
...
namespace TouchPassr {
  ...
  export class JoystickControl extends Control<IJoystickSchema> {
    ...
    protected resetElement(styles: IRootControlStyles): void {
      ...
      var directions: IJoystickDirection[] = (<IJoystickSchema>this.schema)
          .directions,
    ...
    protected triggerDragger(event: DragEvent | MouseEvent): void {
      ...
      var coordinates: number[] = this.getEventCoordinates(event),
        ...
        direction: IJoystickDirection = (<IJoystickSchema>this.schema)
...
namespace TouchPassr {
  ...
  export class TouchPassr implements ITouchPassr {
    ...
    addControl<T extends IControlSchema>(schema: T): void {
      ...
      var control: Control<T> = new TouchPassr.controlClasses[schema.control](
        this.InputWriter,
        schema,
        this.styles,
      );
