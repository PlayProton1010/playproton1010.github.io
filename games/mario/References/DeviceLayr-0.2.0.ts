...
namespace DeviceLayr {
  ...
  export class DeviceLayr implements IDeviceLayr {
    ...
    checkNavigatorGamepads(): number {
      if (
        typeof (<any>navigator).getGamepads === "undefined" ||
        !(<any>navigator).getGamepads()[this.gamepads.length]
      ) {
        return 0;
      }
     ...
     */
    activateGamepadTriggers(gamepad: IGamepad): void {
      var mapping: IControllerMapping =
          DeviceLayr.controllerMappings[gamepad.mapping || "standard"],
        i: number;

      for (
        i = Math.min(mapping.axes.length, gamepad.axes.length) - 1;
        i >= 0;
        i -= 1
      ) {
      ...
      for (
        i = Math.min(mapping.buttons.length, gamepad.buttons.length) - 1;
        i >= 0;
        i -= 1
      ) {
    ...
    activateAxisTrigger(
      ...
      var listing: IJoystickTriggerAxis = (<IJoystickListing>(
          this.triggers[name]
        ))[axis],
        status: AxisStatus;
      ...
      if (
        ...
        listing[AxisStatus[listing.status]] !== undefined
      ) {
        this.InputWritr.callEvent(
          this.aliases.off,
          listing[AxisStatus[listing.status]],
        );
      }
     ...
     */
    private setDefaultTriggerStatuses(
      gamepad: IGamepad,
      triggers: ITriggers,
    ): void {
      var mapping: IControllerMapping =
