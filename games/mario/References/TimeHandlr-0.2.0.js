...
(function (TimeHandlr_1) {
  ...
  var TimeHandlr = (function () {
    ...
    function TimeHandlr(settings) {
      ...
      this.keyOnClassCycleStart =
        settings.keyOnClassCycleStart || "onClassCycleStart";
      this.keyDoClassCycleStart =
        settings.keyDoClassCycleStart || "doClassCycleStart";
      this.keyCycleCheckValidity = settings.keyCycleCheckValidity;
      this.copyCycleSettings =
        typeof settings.copyCycleSettings === "undefined"
          ? true
          : settings.copyCycleSettings;
    ...
    TimeHandlr.prototype.addEvent = function (callback, timeDelay) {
      ...
      var event = new TimeHandlr_1.TimeEvent(
        callback,
        1,
        this.time,
        timeDelay || 1,
     ...
     * @returns An event with the given callback and time information.
     */
    TimeHandlr.prototype.addEventInterval = function (
      callback,
      timeDelay,
      numRepeats,
    ) {
     ...
     * @returns An event with the given callback and time information.
     */
    TimeHandlr.prototype.addEventIntervalSynched = function (
      callback,
      timeDelay,
      numRepeats,
    ) {
      ...
      var calcTime = TimeHandlr_1.TimeEvent.runCalculator(
          timeDelay || this.timingDefault,
        ),
        entryTime = Math.ceil(this.time / calcTime) * calcTime;
      if (entryTime === this.time) {
        return this.addEventInterval.apply(
          this,
          [callback, timeDelay, numRepeats].concat(args),
        );
      } else {
        return this.addEvent.apply(
          this,
          [
     ...
     */
    TimeHandlr.prototype.addClassCycle = function (
      thing,
      settings,
      name,
      timing,
      ...
      settings = thing[this.keyCycles][name || "0"] = this.setClassCycle(
        thing,
        settings,
        timing,
      );
     ...
     * @param timing   A way to determine how long to wait between classes.
     */
    TimeHandlr.prototype.addClassCycleSynched = function (
      thing,
      settings,
      name,
      timing,
    ...
    TimeHandlr.prototype.cancelAllCycles = function (thing) {
      var keyCycles = thing[this.keyCycles],
        cycle,
        name;
     ...
     */
    TimeHandlr.prototype.setClassCycle = function (
      thing,
      settings,
      timing,
      synched,
      ...
      var _this = this;
      timing = TimeHandlr_1.TimeEvent.runCalculator(
      ...
      if (synched) {
        thing[this.keyOnClassCycleStart] = function () {
          var calcTime = settings.length * timing,
            entryDelay =
              Math.ceil(_this.time / calcTime) * calcTime - _this.time,
            event;
          if (entryDelay === 0) {
            event = _this.addEventInterval(
              _this.cycleClass,
              timing,
              Infinity,
              thing,
            ...
            event = _this.addEvent(
              _this.addEventInterval,
              entryDelay,
              _this.cycleClass,
              timing,
              Infinity,
        ...
        thing[this.keyOnClassCycleStart] = function () {
          settings.event = _this.addEventInterval(
            _this.cycleClass,
            timing,
            Infinity,
            thing,
    ...
    TimeHandlr.prototype.cycleClass = function (thing, settings) {
      ...
      if (
        ...
        !settings ||
        !settings.length ||
        (this.keyCycleCheckValidity && !thing[this.keyCycleCheckValidity])
      ) {
        return true;
      }
      var current, name;
    ...
    TimeHandlr.prototype.classRemoveGeneric = function (thing, className) {
      thing[this.keyClassName] = thing[this.keyClassName].replace(
        className,
        "",
      );
    };
