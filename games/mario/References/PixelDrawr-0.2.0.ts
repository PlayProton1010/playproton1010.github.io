  ...
  export class PixelDrawr implements IPixelDrawr {
    ...
    constructor(settings: IPixelDrawrSettings) {
      ...
      this.generateObjectKey =
        settings.generateObjectKey ||
        function (thing: IThing): string {
    ...
    resetBackground(): void {
      this.backgroundCanvas = this.createCanvas(
        this.MapScreener[this.keyWidth],
        this.MapScreener[this.keyHeight],
      );
    ...
    setThingSprite(thing: IThing): void {
      ...
      thing.sprite = this.PixelRender.decode(
    ...
    drawThingOnContext(context: CanvasRenderingContext2D, thing: IThing): void {
      ...
      if (thing.canvas[this.keyWidth] > 0) {
        ...
        this.drawThingOnContextMultiple(
          context,
          thing.canvases,
          thing,
          this.getLeft(thing),
    ...
    private refillThingCanvasSingle(thing: IThing): void {
      ...
      var canvas: HTMLCanvasElement = thing.canvas,
        ...
        imageData: ImageData = context.getImageData(
          0,
          0,
          canvas[this.keyWidth],
          canvas[this.keyHeight],
        );

      // Copy the thing's sprite to that imageData and into the contextz
      this.PixelRender.memcpyU8(
        <Uint8ClampedArray>thing.sprite,
        imageData.data,
      );
    ...
    private refillThingCanvasMultiple(thing: IThing): void {
      ...
      var spritesRaw: PixelRendr.SpriteMultiple = <PixelRendr.SpriteMultiple>(
          thing.sprite
        ),
      ...
      for (i in spritesRaw.sprites) {
        ...
        canvas = this.createCanvas(
          thing.spritewidth * this.unitsize,
          thing.spriteheight * this.unitsize,
        );
      ...
      if (
        thing[this.keyWidth] * thing[this.keyHeight] <
        this.spriteCacheCutoff
      ) {
    ...
    private drawThingOnContextSingle(
      ...
      if (thing.repeat) {
        ...
        context.drawImage(
          ...
          left,
          top,
          canvas.width * thing.scale,
          canvas.height * thing.scale,
        );
    ...
    private drawThingOnContextMultiple(
      ...
      var sprite: PixelRendr.SpriteMultiple = <PixelRendr.SpriteMultiple>(
          thing.sprite
        ),
        topreal: number = top,
      ...
      switch (canvases.direction) {
        ...
        case "vertical":
          ...
          if ((canvasref = <IThingSubCanvas>canvases[this.keyBottom])) {
            diffvert = sprite.bottomheight
              ? sprite.bottomheight * this.unitsize
              : spriteheightpixels;
            this.drawPatternOnContext(
              context,
              canvasref.canvas,
          ...
          if ((canvasref = <IThingSubCanvas>canvases[this.keyTop])) {
            diffvert = sprite.topheight
              ? sprite.topheight * this.unitsize
              : spriteheightpixels;
            this.drawPatternOnContext(
              context,
              canvasref.canvas,
        ...
        case "horizontal":
          ...
          if ((canvasref = canvases[this.keyLeft])) {
            diffhoriz = sprite.leftwidth
              ? sprite.leftwidth * this.unitsize
              : spritewidthpixels;
            this.drawPatternOnContext(
              context,
              canvasref.canvas,
          ...
          if ((canvasref = canvases[this.keyRight])) {
            diffhoriz = sprite.rightwidth
              ? sprite.rightwidth * this.unitsize
              : spritewidthpixels;
            this.drawPatternOnContext(
              context,
              canvasref.canvas,
        ...
        default:
          throw new Error(
            "Unknown or missing direction given in SpriteMultiple.",
          );
      }

      // If there's still room, draw the actual canvas
      if (
        (canvasref = canvases.middle) &&
        topreal < bottomreal &&
        leftreal < rightreal
      ) {
