...
declare namespace GameStartr {
   ...
   */
  export interface IGroupHoldrCustoms
    extends IGameStartrSettingsObject,
      GroupHoldr.IGroupHoldrSettings {}
   ...
   * Settings regarding computations, particularly for an IMathDecidr.
   */
  export interface IMathDecidrCustoms
    extends IGameStartrSettingsObject,
      MathDecidr.IMathDecidrSettings {}
   ...
   * Settings regarding in-game object generation, particularly for an IObjectMakr.
   */
  export interface IObjectMakrCustoms
    extends IGameStartrSettingsObject,
      ObjectMakr.IObjectMakrSettings {}
   ...
   * Settings regarding collision detection, particularily for an IThingHittr.
   */
  export interface IThingHittrCustoms
    extends IGameStartrSettingsObject,
      ThingHittr.IThingHittrSettings {}
   ...
   * Settings regarding touchscreen inputs, particularly for an ITouchPassr.
   */
  export interface ITouchPassrCustoms
    extends IGameStartrSettingsObject,
      TouchPassr.ITouchPassrSettings {}

  /**
   * Settings regarding help text, particularly for an IUsageHelpr.
   */
  export interface IUsageHelprCustoms
    extends IGameStartrSettingsObject,
      UsageHelpr.IUsageHelprSettings {}
  ...
  export interface IGameStartr extends EightBittr.IEightBittr {
     ...
     */
    mapPlaceRandomCommands(
      GameStarter: IGameStartr,
      generatedCommands: WorldSeedr.ICommand[],
    ): void;
...
namespace GameStartr {
  ...
  export class GameStartr extends EightBittr.EightBittr implements IGameStartr {
    ...
    resetUsageHelper(
      ...
      GameStarter.UsageHelper = new UsageHelpr.UsageHelpr(
        GameStarter.settings.help,
      );
    }
    ...
    resetQuadsKeeper(
      ...
      var quadrantWidth: number =
          settings.width / (GameStarter.settings.quadrants.numCols - 3),
        quadrantHeight: number =
          settings.height / (GameStarter.settings.quadrants.numRows - 2);
    ...
    resetGroupHolder(
      ...
      GameStarter.GroupHolder = new GroupHoldr.GroupHoldr(
        GameStarter.settings.groups,
      );
    }
    ...
    resetInputWriter(
      ...
      GameStarter.InputWriter = new InputWritr.InputWritr(
        GameStarter.proliferate(
          {
            canTrigger: GameStarter.canInputsTrigger.bind(
              GameStarter,
              GameStarter,
            ),
    ...
    resetWorldSeeder(
      ...
      GameStarter.WorldSeeder = new WorldSeedr.WorldSeedr(
        GameStarter.proliferate(
          {
            random: GameStarter.NumberMaker.random.bind(
              GameStarter.NumberMaker,
            ),
            onPlacement: GameStarter.mapPlaceRandomCommands.bind(
              GameStarter,
              GameStarter,
            ),
          },
    ...
    resetMathDecider(
      ...
      GameStarter.MathDecider = new MathDecidr.MathDecidr(
        GameStarter.settings.math,
      );
    }
    ...
    resetContainer(
      ...
      GameStarter.canvas = GameStarter.createCanvas(
        settings.width,
        settings.height,
      );
     ...
     */
    scrollWindow(dx: number, dy?: number): void {
      var GameStarter: GameStartr =
        GameStartr.prototype.ensureCorrectCaller(this);
    ...
    thingProcess(
      ...
      num = Math.floor(
        thing.width *
          (thing.GameStarter.unitsize /
            thing.GameStarter.QuadsKeeper.getQuadrantWidth()),
      );
      if (num > 0) {
        maxQuads += ((num + 1) * maxQuads) / 2;
      }
      num = Math.floor(
        (thing.height * thing.GameStarter.unitsize) /
          thing.GameStarter.QuadsKeeper.getQuadrantHeight(),
      );
      if (num > 0) {
        maxQuads += ((num + 1) * maxQuads) / 2;
      }
      ...
      thing.spriteheightpixels =
        thing.spriteheight * thing.GameStarter.unitsize;

      // Canvas, context
      thing.canvas = thing.GameStarter.createCanvas(
        thing.spritewidthpixels,
        thing.spriteheightpixels,
      );
      ...
      if ((cycle = thing.spriteCycle)) {
        thing.GameStarter.TimeHandler.addClassCycle(
          thing,
          cycle[0],
          cycle[1] || null,
          cycle[2] || null,
      ...
      if ((cycle = thing.spriteCycleSynched)) {
        thing.GameStarter.TimeHandler.addClassCycleSynched(
          thing,
          cycle[0],
          cycle[1] || null,
          cycle[2] || null,
      ...
      // Mods!
      thing.GameStarter.ModAttacher.fireEvent(
        "onThingMake",
        thing.GameStarter,
        thing,
        title,
    ...
    shiftBoth(
      ...
      if (!thing.noshiftx) {
        if (thing.parallaxHoriz) {
          thing.GameStarter.shiftHoriz(
            thing,
            thing.parallaxHoriz * dx,
            notChanged,
          );
        } else {
      ...
      if (!thing.noshifty) {
        if (thing.parallaxVert) {
          thing.GameStarter.shiftVert(
            thing,
            thing.parallaxVert * dy,
            notChanged,
          );
        } else {
    ...
    shiftAll(dx: number, dy: number): void {
      var GameStarter: GameStartr =
        GameStartr.prototype.ensureCorrectCaller(this);
      GameStarter.GroupHolder.callAll(
        GameStarter,
        GameStarter.shiftThings,
        dx,
        dy,
        true,
      );
     ...
     */
    addClasses(thing: IThing, ...classes: (string | string[])[]): void {
      var adder: string | string[], i: number, j: number;
    ...
    takeScreenshot(name: string, format: string = "image/png"): void {
      var GameStarter: GameStartr =
          GameStartr.prototype.ensureCorrectCaller(this),
        link: HTMLLinkElement = <HTMLLinkElement>GameStarter.createElement(
          "a",
          {
            download: name + "." + format.split("/")[1],
            href: GameStarter.canvas
              .toDataURL(format)
    ...
    addPageStyles(styles: IPageStyles): void {
      var GameStarter: GameStartr =
          GameStartr.prototype.ensureCorrectCaller(this),
        sheet: HTMLStyleElement = <HTMLStyleElement>GameStarter.createElement(
          "style",
          {
            type: "text/css",
          },
