...
namespace TimeHandlr {
  ...
  export class TimeEvent implements ITimeEvent {
     ...
     */
    static runCalculator(
      value: number | INumericCalculator,
      ...args: any[]
    ): number {
...
namespace TimeHandlr {
  ...
  export class TimeHandlr implements ITimeHandlr {
    ...
    addEvent(
      ...
      var event: TimeEvent = new TimeEvent(
        callback,
        1,
        this.time,
        timeDelay || 1,
    ...
    addEventIntervalSynched(
      ...
      var calcTime: number = TimeEvent.runCalculator(
     ...
     */
    handleEvents(): void {
      var currentEvents: TimeEvent[], i: number;
      ...
      if (synched) {
        thing[this.keyOnClassCycleStart] = (): void => {
          var calcTime: number = settings.length * <number>timing,
            entryDelay: number =
              Math.ceil(this.time / calcTime) * calcTime - this.time,
            event: ITimeEvent;

          if (entryDelay === 0) {
            event = this.addEventInterval(
              this.cycleClass,
              timing,
              Infinity,
              thing,
        ...
        thing[this.keyOnClassCycleStart] = (): void => {
          settings.event = this.addEventInterval(
            this.cycleClass,
            timing,
            Infinity,
            thing,
    ...
    private cycleClass(thing: IThing, settings: ITimeCycle): boolean {
      ...
      var current: boolean | string | IClassCalculator, name: string | boolean;
