...
declare namespace PixelRendr {
  ...
  export interface IGeneralSpriteGenerator {
    (
      ...
      key: string,
      attributes: any,
    ): Uint8ClampedArray | ISpriteMultiple;
  }
  ...
  export interface IPixelRendr {
    ...
    encode(
      ...
      callback: IPixelRendrEncodeCallback,
      ...args: any[]
    ): string;
...
namespace PixelRendr {
  ...
  export class PixelRendr implements IPixelRendr {
    ...
    decode(key: string, attributes: any): Uint8ClampedArray | SpriteMultiple {
      ...
      if (
        !sprite ||
        (<any>sprite.constructor === this.Uint8ClampedArray &&
          (<Uint8ClampedArray>sprite).length === 0)
      ) {
    ...
    private generateSpriteSingleFromRender(
      ...
      var base: Uint8ClampedArray = this.ProcessorBase.process(
          render.source,
          key,
          render.filter,
        ),
        sprite: Uint8ClampedArray = this.ProcessorDims.process(
          base,
          key,
          attributes,
        );
     ...
     * @returns The output sprite.
     */
    private generateSpriteCommandMultipleFromRender(
      render: Render,
      key: string,
      attributes: any,
    ): SpriteMultiple {
    ...
    private generateSpriteCommandSameFromRender(
      ...
      key: string,
      attributes: any,
    ): Uint8ClampedArray | SpriteMultiple {
      var replacement: Render | IRenderLibrary = this.followPath(
    ...
    private generateSpriteCommandFilterFromRender(
      ...
      var filter: IFilter = this.filters[render.source[2]],
        found: Render | IRenderLibrary = this.followPath(
          this.library.sprites,
          render.source[1],
          0,
        ),
      ...
      if (found.constructor === Render) {
        ...
        filtered = this.generateRendersFromFilter(
          <IRenderLibrary>found,
          filter,
        );
      }
    ...
    private generateRendersFromFilter(
      ...
      for (i in directory) {
        ...
        if (child.constructor === Render) {
          output[i] = new Render((<Render>child).source, {
            filter: filter,
          });
        } else {
          output[i] = this.generateRendersFromFilter(
            <IRenderLibrary>child,
            filter,
          );
        }
      }
    ...
    private replaceRenderInContainers(
      ...
      var listing: IRenderContainerListing, i: number;
    ...
    private spriteUnravel(colors: string): string {
      var paletteref: any = this.getPaletteReferenceStarting(
          this.paletteDefault,
        ),
     ...
     */
    private spriteRepeatRows(
      sprite: Uint8ClampedArray,
      key: string,
      attributes: ISpriteAttributes,
    ): Uint8ClampedArray {
      var parsed: Uint8ClampedArray = new this.Uint8ClampedArray(
          sprite.length * this.scale,
        ),
     ...
     *          digit size.
     */
    private imageMapPalette(
      information: [number[], any],
      ...
      var pixels: number[] = information[0],
        ...
        numbers: number[] = <number[]>(
          pixels.map((pixel: number) => paletteIndices[pixel])
        );
     ...
     */
    private imageCombinePixels(
      information: [string[], number[], number],
    ): string {
