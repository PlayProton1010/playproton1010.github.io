  ...
  var UsageHelpr = (function () {
    ...
    UsageHelpr.prototype.displayHelpMenu = function () {
      ...
      this.openings.forEach(function (opening) {
        return _this.logHelpText(opening);
      });
    };
    /**
    ...
    UsageHelpr.prototype.displayHelpOptions = function () {
      ...
      Object.keys(this.options).forEach(function (key) {
        return _this.displayHelpGroupSummary(key);
      });
    ...
    UsageHelpr.prototype.displayHelpGroupSummary = function (optionName) {
      var actions = this.options[optionName],
        action,
        maxTitleLength = 0,
        i;
      ...
      for (i = 0; i < actions.length; i += 1) {
        maxTitleLength = Math.max(
          maxTitleLength,
          this.filterHelpText(actions[i].title).length,
        );
      }
      for (i = 0; i < actions.length; i += 1) {
        action = actions[i];
        this.logger(
          "%c" +
            this.padTextRight(
              this.filterHelpText(action.title),
              maxTitleLength,
            ) +
            "%c  // " +
            action.description,
    ...
    UsageHelpr.prototype.displayHelpOption = function (optionName) {
      ...
      for (i = 0; i < actions.length; i += 1) {
        ...
        if (action.usage) {
          this.logHelpText(["%cUsage: %c" + action.usage, "comment", "code"]);
        }
        if (action.examples) {
      ...
      var message = line[0],
        styles = line
          ...
          .filter(function (style) {
            return UsageHelpr.styles.hasOwnProperty(style);
          })
          .map(function (style) {
            return UsageHelpr.styles[style];
          });
      // A last blank "" style allows the last "%c" in the message to reset text styles
      this.logger.apply(
        this,
        [this.filterHelpText(message)].concat(styles, [""]),
      );
    };
    /**
