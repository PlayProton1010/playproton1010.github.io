...
(function (QuadsKeepr_1) {
  ...
  var QuadsKeepr = (function () {
    ...
    QuadsKeepr.prototype.pushQuadrantRow = function (callUpdate) {
      var row = this.createQuadrantRow(this.left, this.bottom),
        i;
      this.numRows += 1;
    ...
    QuadsKeepr.prototype.pushQuadrantCol = function (callUpdate) {
      var col = this.createQuadrantCol(this.right, this.top),
        i;
      this.numCols += 1;
    ...
    QuadsKeepr.prototype.unshiftQuadrantRow = function (callUpdate) {
      var row = this.createQuadrantRow(
          this.left,
    ...
    QuadsKeepr.prototype.unshiftQuadrantCol = function (callUpdate) {
      var col = this.createQuadrantCol(
    ...
    QuadsKeepr.prototype.determineThingQuadrants = function (thing) {
      var group = thing[this.keyGroupName],
        rowStart = this.findQuadrantRowStart(thing),
        colStart = this.findQuadrantColStart(thing),
        rowEnd = this.findQuadrantRowEnd(thing),
        colEnd = this.findQuadrantColEnd(thing),
        row,
        col;
      ...
      for (row = rowStart; row <= rowEnd; row += 1) {
        for (col = colStart; col <= colEnd; col += 1) {
          this.setThingInQuadrant(
            thing,
            this.quadrantRows[row].quadrants[col],
            group,
          );
        }
      }
     ...
     *                Thing.
     */
    QuadsKeepr.prototype.setThingInQuadrant = function (
      thing,
      quadrant,
      group,
    ) {
    ...
    QuadsKeepr.prototype.createQuadrant = function (left, top) {
      var quadrant = this.ObjectMaker.make("Quadrant"),
        i;
      quadrant[this.keyChanged] = true;
      quadrant.things = {};
      quadrant.numthings = {};
    ...
    QuadsKeepr.prototype.findQuadrantRowStart = function (thing) {
      return Math.max(
        Math.floor((this.getTop(thing) - this.top) / this.quadrantHeight),
        0,
      );
    };
    /**
    ...
    QuadsKeepr.prototype.findQuadrantRowEnd = function (thing) {
      return Math.min(
        Math.floor((this.getBottom(thing) - this.top) / this.quadrantHeight),
        this.numRows - 1,
      );
    };
    /**
    ...
    QuadsKeepr.prototype.findQuadrantColStart = function (thing) {
      return Math.max(
        Math.floor((this.getLeft(thing) - this.left) / this.quadrantWidth),
        0,
      );
    };
    /**
    ...
    QuadsKeepr.prototype.findQuadrantColEnd = function (thing) {
      return Math.min(
        Math.floor((this.getRight(thing) - this.left) / this.quadrantWidth),
        this.numCols - 1,
      );
    };
    return QuadsKeepr;
