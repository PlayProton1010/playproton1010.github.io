...
namespace UsageHelpr {
  ...
  export class UsageHelpr {
     ...
     */
    displayHelpMenu(): void {
      this.openings.forEach((opening: string[]): void =>
        this.logHelpText(opening),
      );
    }
    ...
    displayHelpOptions(): void {
      ...
      Object.keys(this.options).forEach((key: string): void =>
        this.displayHelpGroupSummary(key),
      );
      ...
      var message: string = line[0],
        styles: string[] = (<string[]>line)
          ...
          .filter((style: string): boolean =>
            UsageHelpr.styles.hasOwnProperty(style),
          )
