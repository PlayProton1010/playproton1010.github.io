...
namespace UserWrappr.UISchemas {
  "use strict";
...
namespace UserWrappr.UISchemas {
  ...
  export class ButtonsGenerator
    extends OptionsGenerator
    implements IOptionsGenerator
  {
    /**
    ...
    generate(schema: IOptionsButtonsSchema): HTMLDivElement {
      var output: HTMLDivElement = document.createElement("div"),
        options: IOptionsButtonSchema[] =
          schema.options instanceof Function
      ...
      for (i = 0; i < options.length; i += 1) {
        ...
        element.onclick = function (
          schema: IOptionsButtonSchema,
          element: HTMLDivElement,
        ): void {
          if (
            scope.getParentControlElement(element).getAttribute("active") !==
            "on"
          ) {
            return;
          }
    ...
    protected ensureLocalStorageButtonValue(
      ...
      schema: IOptionsButtonsSchema,
    ): void {
      var key: string = schema.title + "::" + details.title,
        valueDefault: string = details.source
          .call(this, this.GameStarter)
          .toString(),
        value: string;
...
namespace UserWrappr.UISchemas {
   ...
   */
  export class LevelEditorGenerator
    extends OptionsGenerator
    implements IOptionsGenerator
  {
    /**
    ...
    protected createUploaderDiv(): HTMLDivElement {
      ...
      uploader.ondragleave = input.ondragend = this.handleFileDragLeave.bind(
        this,
        uploader,
      );
     ...
     */
    protected createMapSelectorDiv(
      schema: IOptionsEditorSchema,
    ): HTMLDivElement {
      var expanded: boolean = true,
        ...
        toggler: HTMLDivElement = <HTMLDivElement>(
          this.GameStarter.createElement("div", {
            className:
              "select-option select-option-large options-button-option",
          })
        ),
        mapsOut: HTMLDivElement = <HTMLDivElement>(
          this.GameStarter.createElement("div", {
            className:
              "select-options-holder select-options-editor-maps-holder",
          })
        ),
        mapsIn: HTMLDivElement = this.UserWrapper.getGenerators()[
          generatorName
        ].generate(
      ...
      toggler.onclick = function (event?: Event): void {
        ...
        [].slice.call(container.parentElement.children).forEach(function (
          element: HTMLElement,
        ): void {
     ...
     */
    protected handleFileDragEnter(
      uploader: HTMLDivElement,
      event: LevelEditr.IDataMouseEvent,
    ): void {
    ...
    protected handleFileDrop(
      ...
      reader.onprogress = this.handleFileUploadProgress.bind(
        this,
        file,
        uploader,
      );
      reader.onloadend = this.handleFileUploadCompletion.bind(
        this,
        file,
        uploader,
      );
     ...
     */
    protected handleFileUploadProgress(
      file: File,
      uploader: HTMLDivElement,
      event: LevelEditr.IDataProgressEvent,
    ): void {
     ...
     */
    protected handleFileUploadCompletion(
      file: File,
      uploader: HTMLDivElement,
      event: LevelEditr.IDataProgressEvent,
    ): void {
...
namespace UserWrappr.UISchemas {
   ...
   */
  export class MapsGridGenerator
    extends OptionsGenerator
    implements IOptionsGenerator
  {
    /**
      ...
      for (i = rangeY[0]; i <= rangeY[1]; i += 1) {
        ...
        for (j = rangeX[0]; j <= rangeX[1]; j += 1) {
          ...
          cell.onclick = function (callback: () => any): void {
            if (
              scope.getParentControlElement(cell).getAttribute("active") ===
              "on"
            ) {
              callback();
            }
            ...
            scope,
            schema.callback.bind(scope, scope.GameStarter, schema, cell),
          );
    ...
    appendExtras(output: HTMLDivElement, schema: IOptionsMapGridSchema): void {
      ...
      for (i = 0; i < schema.extras.length; i += 1) {
        ...
        element.onclick = extra.callback.bind(
          this,
          this.GameStarter,
          schema,
          element,
...
namespace UserWrappr.UISchemas {
  ...
  export interface IOptionsTableTypes {
    [i: string]: (
      input: IInputElement | ISelectElement,
      details: IOptionsTableOption,
      schema: ISchema,
    ) => any;
  }
    ...
    implements IOptionsGenerator
  {
    ...
    generate(schema: IOptionsTableSchema): HTMLDivElement {
      ...
      if (schema.options) {
        for (i = 0; i < schema.options.length; i += 1) {
          ...
          child = TableGenerator.optionTypes[schema.options[i].type].call(
            this,
            input,
            option,
            schema,
     ...
     */
    protected setBooleanInput(
      input: IInputElement,
      details: IOptionsTableBooleanOption,
      schema: ISchema,
    ): IInputElement {
      ...
      input.className =
        "select-option options-button-option option-" + statusClass;
     ...
     * @returns An HTML element containing the input.
     */
    protected setKeyInput(
      input: IInputElement,
      details: IOptionsTableKeysOption,
      schema: ISchema,
    ): ISelectElement[] {
     ...
     * @returns An HTML element containing the input.
     */
    protected setNumberInput(
      input: IInputElement,
      details: IOptionsTableNumberOption,
      schema: ISchema,
    ): IInputElement {
      var child: IInputElement = <UISchemas.IInputElement>(
      ...
      child.type = "number";
      child.value = Number(
        details.source.call(scope, scope.GameStarter),
      ).toString();
      child.min = (details.minimum || 0).toString();
      child.max = (
        details.maximum || Math.max(details.minimum + 10, 10)
      ).toString();
    ...
    protected setSelectInput(
      input: ISelectElement,
      details: IOptionsTableSelectOption,
      schema: ISchema,
    ): ISelectElement {
      var child: ISelectElement = <ISelectElement>(
     ...
     * @returns An HTML element containing the input.
     */
    protected setScreenSizeInput(
      input: ISelectElement,
      details: IOptionsTableScreenSizeOption,
      schema: ISchema,
    ): ISelectElement {
     ...
     */
    protected ensureLocalStorageInputValue(
      childRaw: IChoiceElement | IChoiceElement[],
      details: IOption,
      schema: ISchema,
    ): void {
      if (childRaw.constructor === Array) {
        this.ensureLocalStorageValues(
          <IInputElement[]>childRaw,
          details,
          schema,
        );
        return;
      ...
      var child: IInputElement | ISelectElement = <
          IInputElement | ISelectElement
        >childRaw,
     ...
     * @param schema   The container schema this child is within.
     */
    protected ensureLocalStorageValues(
      children: (IInputElement | ISelectElement)[],
      details: IOption,
      schema: ISchema,
    ): void {
     ...
     */
    protected storeLocalStorageValue(
      child: IInputElement | ISelectElement,
      value: any,
    ): void {
...
namespace UserWrappr {
  ...
  export class UserWrappr {
     ...
     */
    private documentElement: HTMLHtmlElement = <HTMLHtmlElement>(
    ...
    constructor(settings: IUserWrapprSettings) {
      ...
      this.allPossibleKeys =
        settings.allPossibleKeys || UserWrappr.allPossibleKeys;
    ...
    setCurrentSize(size: string | ISizeSummary): void {
      if (typeof size === "string" || size.constructor === String) {
        if (!this.sizes.hasOwnProperty(<string>size)) {
          throw new Error(
            "Size " + size + " does not exist on the UserWrappr.",
          );
        }
    ...
    private importSizes(sizesRaw: ISizeSummaries): ISizeSummaries {
      var sizes: ISizeSummaries =
          this.GameStartrConstructor.prototype.proliferate({}, sizesRaw),
        i: string;
    ...
    private fixCustoms(customsRaw: IGameStartrCustoms): any {
      var customs: IGameStartrCustoms =
        this.GameStartrConstructor.prototype.proliferate({}, customsRaw);
    ...
    private loadGameStarter(customs: IGameStartrCustoms): void {
      var section: HTMLElement = <HTMLElement>(
        document.querySelector(this.gameElementSelector)
      );
    ...
    private loadControls(schemas: UISchemas.ISchema[]): void {
      var section: HTMLElement = <HTMLElement>(
          document.querySelector(this.gameControlsSelector)
        ),
