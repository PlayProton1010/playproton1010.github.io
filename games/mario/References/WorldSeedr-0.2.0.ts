...
namespace WorldSeedr {
  ...
  export class SpacingCalculator implements ISpacingCalculator {
     ...
     */
    constructor(
      randomBetween: IRandomNumberGenerator,
      chooseAmong: IOptionChooser<IPossibilitySpacingOption>,
    ) {
      ...
      switch (spacing.constructor) {
        case Array:
          ...
          return this.calculateFromPossibilities(
            <IPossibilitySpacingOption[]>spacing,
          );
...
namespace WorldSeedr {
  ...
  export class WorldSeedr {
    ...
    private generateMultiple(
      ...
      return contents.children.map(function (
        choice: IPossibilityChild,
      ): IChoice {
        var output: IChoice = scope.parseChoice(
          choice,
        ...
        if (direction) {
          (<WorldSeedr>scope).movePositionBySpacing(
            position,
            direction,
            spacing,
          );
    ...
    private generateChild(
      ...
      var choice: IPossibilityChild = this.chooseAmongPosition(
        contents.children,
        position,
      );
        ...
        output: IChoice = {
          ...
          arguments:
            choice.arguments instanceof Array
              ? (<IArgumentPossibility>this.chooseAmong(choice.arguments))
                  .values
              : choice.arguments,
          width: undefined,
          height: undefined,
    ...
    private chooseAmongPosition(
      ...
      return this.chooseAmong(
        choices.filter(function (choice: IPossibilityChild): boolean {
    ...
    private wrapChoicePositionExtremes(children: IChoice[]): IChoice {
      var position: IChoice, child: IChoice, i: number;
     ...
     * @param position   An Object that contains .left, .right, .top, and .bottom.
     */
    private ensureDirectionBoundsOnChoice(
      output: IChoice,
      position: IPosition,
    ): void {
      var i: string;
