...
(function (PixelRendr_1) {
  ...
  var PixelRendr = (function () {
    ...
    PixelRendr.prototype.decode = function (key, attributes) {
      ...
      if (
        !sprite ||
        (sprite.constructor === this.Uint8ClampedArray && sprite.length === 0)
      ) {
     ...
     *          giveArrays is true).
     */
    PixelRendr.prototype.generatePaletteFromRawData = function (
      data,
      forceZeroColor,
      giveArrays,
    ) {
     ...
     */
    PixelRendr.prototype.memcpyU8 = function (
      source,
      destination,
      readloc,
      writeloc,
     ...
     *                     generation process.
     */
    PixelRendr.prototype.generateRenderSprite = function (
      render,
      key,
      attributes,
    ) {
      ...
      if (render.source.constructor === String) {
        ...
        sprite = this.commandGenerators[render.source[0]](
          render,
          key,
          attributes,
        );
     ...
     */
    PixelRendr.prototype.generateSpriteSingleFromRender = function (
      render,
      key,
      attributes,
    ) {
      var base = this.ProcessorBase.process(render.source, key, render.filter),
        sprite = this.ProcessorDims.process(base, key, attributes);
      return sprite;
    };
    /**
     ...
     * @returns The output sprite.
     */
    PixelRendr.prototype.generateSpriteCommandMultipleFromRender = function (
      render,
      key,
      attributes,
    ) {
      var sources = render.source[2],
        ...
        sprite,
        path,
        output = new PixelRendr_1.SpriteMultiple(sprites, render),
        i;
      for (i in sources) {
     ...
     * @returns The output sprite.
     */
    PixelRendr.prototype.generateSpriteCommandSameFromRender = function (
      render,
      key,
      attributes,
    ) {
      var replacement = this.followPath(
     ...
     * @returns The output sprite.
     */
    PixelRendr.prototype.generateSpriteCommandFilterFromRender = function (
      render,
      key,
      attributes,
    ) {
      var filter = this.filters[render.source[2]],
        found = this.followPath(this.library.sprites, render.source[1], 0),
        filtered;
      if (!filter) {
     ...
     */
    PixelRendr.prototype.generateRendersFromFilter = function (
      directory,
      filter,
    ) {
      var output = {},
     ...
     */
    PixelRendr.prototype.replaceRenderInContainers = function (
      render,
      replacement,
    ) {
      var listing, i;
        ...
        switch (colors[loc]) {
          ...
          case "p":
            ...
            if (colors[++loc] === "[") {
              ...
              paletteref = this.getPaletteReference(
                colors.slice(loc + 1, nixloc).split(","),
              );
              loc = nixloc + 1;
     ...
     */
    PixelRendr.prototype.spriteApplyFilter = function (
      colors,
      key,
      attributes,
    ) {
    ...
    PixelRendr.prototype.spriteGetArray = function (colors) {
      var clength = colors.length,
        ...
        olength = numcolors * 4,
        output = new this.Uint8ClampedArray(olength),
        reference,
        i,
        j,
        k;
    ...
    PixelRendr.prototype.spriteRepeatRows = function (sprite, key, attributes) {
      var parsed = new this.Uint8ClampedArray(sprite.length * this.scale),
        ...
        height = attributes[this.spriteHeight] / this.scale,
        readloc = 0,
        writeloc = 0,
        i,
        j;
     ...
     */
    PixelRendr.prototype.spriteFlipDimensions = function (
      sprite,
      key,
      attributes,
    ) {
    ...
    PixelRendr.prototype.flipSpriteArrayHoriz = function (sprite, attributes) {
      var length = sprite.length + 0,
        width = attributes[this.spriteWidth] + 0,
        newsprite = new this.Uint8ClampedArray(length),
        rowsize = width * 4,
        newloc,
        oldloc,
        i,
    ...
    PixelRendr.prototype.imageGetPixels = function (data) {
      ...
      for (i = 0, j = 0; i < data.length; i += 4, j += 1) {
        pixel = this.getClosestInPalette(
          this.paletteDefault,
          data.subarray(i, i + 4),
        );
        pixels[j] = pixel;
    ...
    PixelRendr.prototype.imageMapPalette = function (information) {
      var pixels = information[0],
        ...
        digitsize = this.getDigitSizeFromArray(palette),
        paletteIndices = this.getValueIndices(palette),
        numbers = pixels.map(function (pixel) {
          return paletteIndices[pixel];
        });
    ...
    PixelRendr.prototype.imageCombinePixels = function (information) {
      ...
      output =
        "p[" +
        palette.map(this.makeSizedDigit.bind(this, digitsize)).join(",") +
        "]";
    ...
    PixelRendr.prototype.getPaletteReference = function (palette) {
      ...
      for (i = 0; i < palette.length; i += 1) {
        output[this.makeDigit(i, digitsize)] = this.makeDigit(
          palette[i],
          digitsize,
        );
      }
    ...
    PixelRendr.prototype.makeDigit = function (num, size, prefix) {
      ...
      return (
        this.stringOf(prefix, Math.max(0, size - String(num).length)) + num
      );
    };
    /**
