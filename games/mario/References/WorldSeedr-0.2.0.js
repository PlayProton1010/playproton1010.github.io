  ...
  var SpacingCalculator = (function () {
    ...
    SpacingCalculator.prototype.calculateFromPossibility = function (spacing) {
      var spacingObject = spacing,
        min = spacingObject.min,
        max = spacingObject.max,
        units = spacingObject.units || 1;
     ...
     * @returns A valid distance for the given spacing description.
     */
    SpacingCalculator.prototype.calculateFromPossibilities = function (
      spacing,
    ) {
...
(function (WorldSeedr_1) {
  ...
  var WorldSeedr = (function () {
    ...
    function WorldSeedr(settings) {
      ...
      this.onPlacement =
        settings.onPlacement || console.log.bind(console, "Got:");
      this.spacingCalculator = new WorldSeedr_1.SpacingCalculator(
    ...
    WorldSeedr.prototype.generate = function (name, command) {
      ...
      if (!schema.contents) {
        throw new Error(
          "Possibility '" + name + "' has no possibile outcomes.",
        );
      }
     ...
     *          position and some number of children.
     */
    WorldSeedr.prototype.generateChildren = function (
      schema,
      position,
      direction,
    ) {
      var contents = schema.contents,
        ...
        objectMerged = this.objectMerge(schema, position),
        children;
      direction = contents.direction || direction;
      switch (contents.mode) {
        case "Random":
          children = this.generateRandom(
            contents,
            objectMerged,
            direction,
            spacing,
        ...
        case "Repeat":
          ...
          break;
        case "Multiple":
          children = this.generateMultiple(
            contents,
            objectMerged,
            direction,
            spacing,
     ...
     */
    WorldSeedr.prototype.generateCertain = function (
      contents,
      position,
      direction,
      spacing,
     ...
     *          and some number of children.
     */
    WorldSeedr.prototype.generateRepeat = function (
      contents,
      position,
      direction,
      spacing,
     ...
     *          and some number of children.
     */
    WorldSeedr.prototype.generateRandom = function (
      contents,
      position,
      direction,
      spacing,
     ...
     *          and some number of children.
     */
    WorldSeedr.prototype.generateMultiple = function (
      contents,
      position,
      direction,
      spacing,
      ...
      return contents.children.map(function (choice) {
        var output = scope.parseChoice(
          choice,
          scope.objectCopy(position),
          direction,
        );
        if (direction) {
     ...
     *          .arguments.
     */
    WorldSeedr.prototype.generateChild = function (
      contents,
      position,
      direction,
    ) {
    ...
    WorldSeedr.prototype.parseChoice = function (choice, position, direction) {
      var title = choice.title,
        schema = this.possibilities[title],
        output = {
          title: title,
          type: choice.type,
          arguments:
     ...
     */
    WorldSeedr.prototype.parseChoiceFinal = function (
      choice,
      position,
      direction,
    ) {
    ...
    WorldSeedr.prototype.chooseAmongPosition = function (choices, position) {
      ...
      return this.chooseAmong(
        choices.filter(function (choice) {
          return scope.choiceFitsSize(
            scope.possibilities[choice.title],
            width,
            height,
          );
        }),
     ...
     *                    (by default, 0).
     */
    WorldSeedr.prototype.shrinkPositionByChild = function (
      position,
      child,
      direction,
      spacing,
     ...
     *                    (by default, 0).
     */
    WorldSeedr.prototype.movePositionBySpacing = function (
      position,
      direction,
      spacing,
    ) {
     ...
     */
    WorldSeedr.prototype.ensureSizingOnChoice = function (
      output,
      choice,
      schema,
    ) {
      ...
      for (i in sizingNames) {
        ...
        name = sizingNames[i];
        output[name] =
          choice.sizing && typeof choice.sizing[name] !== "undefined"
            ? choice.sizing[name]
            : schema[name];
      }
    };
     ...
     * @param position   An Object that contains .left, .right, .top, and .bottom.
     */
    WorldSeedr.prototype.ensureDirectionBoundsOnChoice = function (
      output,
      position,
    ) {
      var i;
