...
scene("game", () => {
  if (STUFF.achievements == undefined) STUFF.achievements = [];
  ...
  var collectableSizes = [
    0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.9, 0.9, 0.9, 0.9, 1.3, 1.3, 1.3, 1.3,
  ];
  ...
  onUpdate(() => {
    if (chickenShot && !stopped) {
      ...
      velocityMach = (
        Math.sqrt(chicken.xv ** 2 + chicken.yv ** 2) / 343
      ...
      if (Math.abs(chicken.xv) <= 0.2) chicken.xv = 0;

      distanceF = chicken.pos.x / SCALE;
      every("distanceText", (d) => {
        d.text = `${shortNum(distanceF)}m travelled`;
      });
      distance = Math.floor(distanceF);

      altitude = (height() - SCALE * 1.1 - chicken.pos.y) / SCALE;
      every("altitudeText", (a) => {
        a.text = `Altitude: ${shortNum(altitude)}m`;
      });

      maxAltitude = Math.max(maxAltitude, altitude);
      every("maxAltitudeText", (m) => {
        m.text = `Top Altitude: ${shortNum(maxAltitude)}m`;
      });

      every("velocityText", (v) => {
        let val =
          velocityMach >= 0.8
            ? `Mach ${parseFloat(velocityMach).toFixed(1)}`
            : `${shortNum(velocity)} m/s`;
        v.text = `Velocity: ${val}`;
      });
      ...
      if (distance % 2 == 0 && lastStar != distance) {
        ...
        for (let s = 0; s < STAR_TYPES.length; s++) {
          ...
          for (let i = 0; i < current.count; i++) {
            ...
            if (
              Math.max(Math.abs(chicken.yv), 10) >
      ...
      if (isMouseDown() || isKeyDown("space")) {
        if (fuel > 0) {
          ...
          add([
            ...
            pos(
              chicken.pos.add(SCALE * rand(-0.1, 0.1), SCALE * rand(-0.1, 0.1)),
            ),
          ]);
        } else {
          detatchAnim();
    ...
    if (debugMode) {
      ...
      camScale(
        Math.max(Math.min((-1 / 3000) * altitude + 1 + 20 / 3000, 1), 0.35),
      );
    }

    if (debugMode)
      debug.log(
        ` add: ${starsRendered} / ${starsPossible} \n obj: ${debug.objCount()}\n fps: ${debug.fps()} ( -${60 - debug.fps()} ) \nzoom: ${debugZoom.toFixed(1)}`,
      );
  });
    ...
    if (!endMenuShown) {
      ...
      setTimeout(() => {
        ...
        let moneyGain = Math.floor(
          score / 2 + getScore(distanceF - 16) + getScore(maxAltitude - 3),
        );
