scene("retry", () => {
  add([rect(width(), height()), pos(0, 0), color(BLACK)]);
  go("game");
});
...
scene("shop", () => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    add([sprite("shopRoof"), scale(2 * TILE), pos(i * 2 * SCALE, 0), z(Z.top)]);
  }
  ...
  function updateShop() {
    ...
    every("shopButton", (b) => {
      ...
      if (selectedOffer == b.btn) {
        ...
        buyBtn.color = affordable ? rgb(0, c1, 0) : rgb(c1, c1, c1);
        buyBtnInner.color = affordable ? rgb(0, c2, 0) : rgb(c2, c2, c2);
      }
    });
